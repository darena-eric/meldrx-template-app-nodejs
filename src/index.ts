...
const WORKSPACE_URL = process.env.MELDRX_WORKSPACE_URL ?? "";
const KID = "1234567890";
const ALG = "RS384";
const PRIVATE_KEY = fs.readFileSync(__dirname + "/../privatekey.pem").toString();

async function main() {
    const workspaceId = WORKSPACE_URL.split("/").pop() ?? "";
    const fhirUrl = MeldRxUtils.getWorkspaceFhirUrl(AUTH_URL, workspaceId);
    const tokenUrl = MeldRxUtils.getWorkspaceTokenUrl(AUTH_URL, workspaceId);
